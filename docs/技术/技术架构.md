...
当前采购、销售和双边合同流程已经存储在 `ApprovalFlow`、`ApprovalFlowNode`，支持流程启停、固定审批人、节点启停和采购金额门槛。下一阶段补齐流程及节点增删排序、角色审批人和流程版本，使日常调整不再修改代码。

权限项与审批配置必须区分：

- 角色、已有权限组合、审批模板、节点顺序和审批人属于管理配置；
- 新权限编码、新审批业务对象、新条件算法、新审批动作和会签/并行能力属于开发；
- 在途审批绑定提交时的流程版本或节点快照，模板变化只影响后续提交。

审批人解析必须读取有效多角色和对应审批权限，不能继续仅依赖兼容字段 `User.role`。系统管理员保留全部当前节点的兜底处理能力，并记录实际操作人。
