...
3. **业务链紧凑：** 合同 → 执行批次 → 执行通知 → 物流运单 → 库存/质检 → 结算是紧密编排的业务链，分布式事务会大幅增加复杂度
...
- **复杂关联查询**：大宗贸易涉及合同-执行批次-执行通知-物流运单-出入库单-结算单的多层关联，Prisma 的 `include`/`select` 处理嵌套读取更直观
...
**1. 多主体隔离 — 角色授权数据范围与单据链继承**

当前应用层使用 `Role`、`Permission`、`UserRoleAssignment`、`UserRoleScope` 计算功能权限和数据范围。唯一平台管理主体承载部门、员工、账号和业务单元；业务单元负责内部经营归属、利润及权限；合作伙伴中的内部签约主体负责法律签约和库存货权。合同以卖方、买方和签约主体确定外部企业关联，以业务单元确定内部范围；下游单据沿来源关系继承。后续可使用 PostgreSQL Row-Level Security 加固关键表。
...
GET    /api/v1/dispatch-notices/orders/:id/availability # 执行批次剩余可通知数量
POST   /api/v1/dispatch-notices             # 创建供应商发货指令或销售发货通知单
PATCH  /api/v1/dispatch-notices/:id/status  # 执行通知下达/取消/完成
GET    /api/v1/waybills/dispatch-notices/:id/availability # 执行通知剩余可运输数量
POST   /api/v1/waybills                     # 创建物流运单
POST   /api/v1/waybills/batch               # 按执行通知批量创建独立物流运单
PATCH  /api/v1/waybills/:id/assignment      # 车辆与司机调度
PATCH  /api/v1/waybills/:id/status          # 待发运/在途/到达/签收状态流转
```
...
**JWT + 多角色 RBAC（功能权限） + 用户角色数据范围（企业/部门/本人）+ 单据链范围继承，预留 SSO/OAuth2 扩展点。**
...
// Permission.code 使用“资源.操作”，例如：
// contract.view / contract.approve
// execution.view / execution.manage
...
- 一个用户可有多个有效角色授权，功能权限和范围分别合并计算。
- 内部用户支持全部、本企业、指定企业、部门、部门及下级、本人范围。
- 外部用户必须绑定 `EXTERNAL Company`，数据范围强制为本企业，且 `Company.partnerId` 必须存在。
- 外部企业范围从合同 `sellerId`、`buyerId`、`signingPartnerId` 匹配开始，下游通过 Prisma 关系过滤继承。
- 列表、详情、候选单据、写操作和附件接口均由服务端注入相同范围。
- 外部企业账号不得授予 `ADMIN`；系统管理员拥有全部范围。
- 二期可在数据库层对关键表增加 RLS，作为应用层范围校验的纵深防御。

完整实现见[角色权限与数据范围设计](./模块设计/角色权限与数据范围.md)。
...
const CONTRACT_TRANSITIONS: StateTransition[] = [
  ...
  },
  {
    ...
    sideEffect: async (c) => { /* 合同生效，允许创建执行批次 */ },
  },
  {
  ...
  },
  {
    ...
    allowedRoles: ['system'], // 系统自动：首次生成执行批次后
  },
  {
...
| 入库作业单 | 待到货（采购运单确认发运时自动生成）→ 到达/签收/过磅/质检作业中 → 合格待收货 → 已收货 → 已入账 |
| 结算单 | 待生成 → 待审核 → 已审核 → 已付款 |
| 发票 | 待开具 → 已开具 → 已认证 |

---

### 9.5 审批流程：轻量配置，不自研通用 BPM

**核心原则：采用数据库驱动的顺序审批模板，满足日常配置；不建设可视化通用 BPM 引擎。**

当前采购、销售和双边合同流程存储在 `ApprovalFlow`、`ApprovalFlowNode`。节点绑定审批角色，支持会签/或签、合同部门/合同企业/全平台人员范围、节点启停和采购金额门槛。合同提交时解析有效多角色授权并生成成员快照；在途任务不受后续角色成员变化影响。下一阶段补齐流程及节点增删排序和流程版本。

权限项与审批配置必须区分：

- 角色、已有权限组合、审批模板、节点顺序和审批人属于管理配置；
- 新权限编码、新审批业务对象、新条件算法、新审批动作和并行能力属于开发；
- 在途审批绑定提交时的流程版本或节点快照，模板变化只影响后续提交。

审批人解析读取有效多角色、授权有效期、组织数据范围和 `contract.approve` 权限，不依赖兼容字段 `User.role`。系统管理员保留全部当前节点的兜底处理能力，并记录每条成员任务的原审批人与实际操作人。

当业务出现大量复杂分支、会签/或签、跨系统编排或高频流程建模需求时，再评估接入成熟工作流引擎（如 Flowable），避免自行实现完整 BPM。
...
当前一期已在小程序质检任务中落地“拍摄样品照片 → 同步 OCR 识别标签文字 → 回填样品标签、封签编号、取样方法和送检机构 → 人工核对 → 保存样品与照片”。该功能使用百度 OCR，由 `BAIDU_OCR_API_KEY` 和 `BAIDU_OCR_SECRET_KEY` 配置；未配置或第三方服务异常时，仅降级为手工录入，不阻断现场拍照及样品归档。

磅单、发票、合同等通用单据仍按以下异步流水线逐步接入：
...
- OCR 结果与原始图片绑定存储
- 现场样品照片为必需证据，OCR 只辅助填写，不代替操作人员确认
- 低置信度字段高亮标记，引导人工修正
- 修正数据反馈到训练集（二期模型升级）
...
打通“合同 → 执行批次 → 执行通知 → 物流运单 → 地磅/质检 → 库存 → 结算”的最小闭环。
...
| **P0** | 双边合同 | 创建 + 在同一合同下生成采购/销售执行批次 |
| **P0** | 供应商发货指令/销售发货通知单 | 供应商发货指令、销售发货通知单生成 |
| **P0** | 物流运单 | 创建/查询/状态流转 |
| **P0** | 物流入库单 | 采购运单确认发运后自动生成入库作业单，持续跟踪到货、签收、过磅、质检、收货与入账 |
| **P0** | 物流出库单 | 常规出库确认 |
| **P0** | 地磅管理 | 磅单上传/关联/查询 |
| **P0** | 业务入库单 | 物流收货确认后生成业务入库、库存批次和台账 |
| **P0** | 销售出库单 | 销售发货通知单 → 出库 |
