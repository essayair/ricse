...
当前应用层使用 `Role`、`Permission`、`UserRoleAssignment`、`UserRoleScope` 计算功能权限和数据范围。合同以卖方、买方和签约主体确定外部企业关联；下游单据沿来源关系继承合同范围。部门组保留为业务归组和报表维度，不作为当前唯一鉴权键。后续可使用 PostgreSQL Row-Level Security 加固关键表。
