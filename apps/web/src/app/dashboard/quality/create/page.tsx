...
function Conclusion({ value }: { value: string }) { const labels: Record<string, string> = { PENDING: '待补充数据', PASS: '合格', DEDUCTION: '不合格（超标扣款）', FUSE: '不合格（拒收）' }; return <div className={`mt-1 font-semibold ${value === 'FUSE' ? 'text-destructive' : value === 'PASS' ? 'text-primary' : value === 'DEDUCTION' ? 'text-amber-600' : ''}`}>{labels[value]}</div>; }
