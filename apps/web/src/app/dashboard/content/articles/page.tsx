...
import { Edit3, ExternalLink, ImageUp, Newspaper, Plus, Search, Send, Tags, Trash2, X } from 'lucide-react';
...
const EMPTY = {
  ...
  ingestionMode: 'MANUAL', sourceUrl: '', sourcePublishedAt: '', collectedAt: '', reviewNote: '',
};
...
export default function ContentArticlesPage() {
  ...
  const beginEdit = (item: any) => {
    ...
    setForm({
      ...
      ingestionMode: item.ingestionMode || 'MANUAL',
  ...
  const changeStatus = async (item: any, next: string) => {
    ...
    if (next === 'REJECTED') {
      const reason = window.prompt(`请输入驳回“${item.title}”的审核意见：`);
      if (reason === null) return;
      if (!reason.trim()) return alert('驳回时必须填写审核意见');
      reviewNote = reason.trim();
    } else if (!window.confirm(next === 'PUBLISHED'
      ? `确定审核通过并发布“${item.title}”吗？发布后官网和小程序将可以读取。`
      : `确定下线“${item.title}”吗？`)) return;
    try { await api.patch(`/content/articles/${item.id}/status`, { status: next, reviewNote }); load(); } catch (error: any) { alert(error.message); }
  };
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h1 className="text-2xl font-bold">资讯管理</h1><p className="mt-1 text-sm text-muted-foreground">人工内容保存为草稿；生意社自动采集内容去重后直接发布，可在此编辑或下线</p></div>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <Card className="p-4"><div className="text-xs text-muted-foreground">待处理内容</div><div className="mt-1 text-2xl font-bold text-blue-600">{data.list.filter((item: any) => item.status === 'PENDING_REVIEW').length}</div></Card>
      <Card className="p-4"><div className="text-xs text-muted-foreground">草稿 / 驳回 / 下线</div><div className="mt-1 text-2xl font-bold text-amber-600">{data.list.filter((item: any) => ['DRAFT', 'REJECTED', 'OFFLINE'].includes(item.status)).length}</div></Card>
    </div>
    ...
    <Card className="overflow-hidden">
      {!data.list.length ? <Empty icon={<Newspaper className="h-8 w-8" />} text="暂无内容" /> : <div className="overflow-x-auto"><table className="w-full min-w-[1250px] text-sm">
        <thead className="border-b bg-muted/50 text-left text-muted-foreground"><tr><th className="p-3">标题 / 摘要</th><th className="p-3">类型</th><th className="p-3">栏目</th><th className="p-3">来源 / 采集方式</th><th className="p-3">来源 / 发布时间</th><th className="p-3">状态</th><th className="p-3 text-right">浏览 / 点赞</th><th className="p-3">操作</th></tr></thead>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto"><DialogHeader><DialogTitle>{editingId ? '编辑内容' : '新建内容'}</DialogTitle></DialogHeader>
      ...
      {form.ingestionMode === 'AUTO' && <div className="rounded-md border border-blue-200 bg-blue-50 p-3 text-xs leading-5 text-blue-900"><div className="font-medium">自动采集资讯</div><div>来源时间：{contentDate(form.sourcePublishedAt)}　采集时间：{contentDate(form.collectedAt)}</div>{form.sourceUrl && <a href={form.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex items-center text-primary hover:underline">查看生意社原文 <ExternalLink className="ml-1 h-3.5 w-3.5" /></a>}{form.reviewNote && <div className="mt-1 text-destructive">历史处理意见：{form.reviewNote}</div>}</div>}
