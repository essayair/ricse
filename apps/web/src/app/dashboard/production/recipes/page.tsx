...
export default function ProductionRecipesPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="overflow-hidden">{!items.length ? <div className="p-12 text-center text-muted-foreground">暂无生产方案，请先维护投入与产出物料后创建</div> : <div className="overflow-x-auto"><table className="w-full min-w-[1380px] text-sm"><thead className="border-b bg-muted/50 text-left text-muted-foreground"><tr><th className="p-3">方案编号 / 名称</th><th className="p-3">库存主体</th><th className="p-3">产出物料</th><th className="p-3 text-right">基准产量</th><th className="p-3">标准投入</th><th className="p-3 text-right">目标收率</th><th className="p-3 text-right">损耗容差</th><th className="p-3">质量控制</th><th className="p-3">引用任务</th><th className="p-3">状态</th><th className="p-3">操作</th></tr></thead><tbody>
      {items.map(item => <tr key={item.id} className="border-b"><td className="p-3"><div className="font-mono font-medium text-primary">{item.recipeNo}</div><div className="mt-1 font-medium">{item.name}</div></td><td className="p-3">{item.ownerPartner.name}</td><td className="p-3"><div>{item.outputMaterial.name}</div><div className="mt-1 font-mono text-xs text-muted-foreground">{item.outputMaterial.code}</div></td><td className="p-3 text-right">{quantity(item.baseOutputQuantity, item.outputMaterial.unit)}</td><td className="max-w-96 p-3">{item.inputs.map((input: any) => <div key={input.id} className="mb-1"><span className="text-xs text-muted-foreground">{MATERIAL_ROLE[input.materialRole] || input.materialRole}</span> · {input.material.name} {quantity(input.quantity, input.unit)}</div>)}</td><td className="p-3 text-right">{percent(item.expectedYieldRate)}</td><td className="p-3 text-right">±{percent(item.lossToleranceRate)}</td><td className="p-3">{item.qualityRequired ? '完工需质检' : '免质检'}</td><td className="p-3">{item._count.tasks}</td><td className="p-3"><StatusText status={item.status}>{item.status === 'ACTIVE' ? '启用' : '停用'}</StatusText></td><td className="p-3"><Link href={`/dashboard/production/recipes/${item.id}/edit`}><Button size="sm" variant="outline"><Pencil className="mr-1 h-3 w-3" />编辑</Button></Link></td></tr>)}
