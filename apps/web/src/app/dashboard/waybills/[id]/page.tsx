...
import { BusinessDirectionBadge, waybillStatusLabel, waybillTransitionLabel } from '@/components/business-direction';
...
const RECEIPT_ATTACHMENT_CATEGORIES = ['RECEIPT', 'RECEIPT_DOCUMENT', 'RECEIPT_PHOTO', 'RECEIPT_OTHER'];
const isReceiptAttachment = (category: string) => RECEIPT_ATTACHMENT_CATEGORIES.includes(category);
export default function WaybillDetailPage() {
  ...
  const [canViewLogisticsContract, setCanViewLogisticsContract] = useState(false);
  ...
  useEffect(() => {
    ...
    try {
      ...
      setCanViewLogisticsContract(user.role === 'ADMIN' || (user.permissions || []).includes('logistics.contract.view'));
    } catch {
      setCanViewLogisticsContract(false);
    }
    Promise.all([api.get<{ items: Vehicle[] }>('/partners/vehicles?status=ACTIVE&pageSize=100'), api.get<{ items: CarrierProfile[] }>('/service-organizations?type=LOGISTICS_CARRIER&status=ACTIVE&pageSize=200'), api.get<{ items: Driver[] }>('/drivers?status=ACTIVE&pageSize=200')]).then(([vehicleData, carrierData, driverData]) => { setVehicles(vehicleData.items); setCarriers(carrierData.items || []); setDrivers(driverData.items || []); });
  }, [id]);
  ...
  const sales = waybill.dispatchNotice.type === 'SALES';
  const actions: Record<string, Array<{ status: string; variant?: 'default' | 'destructive' | 'outline' }>> = {
    PENDING: [{ status: 'IN_TRANSIT' }, { status: 'CANCELLED', variant: 'destructive' }],
    IN_TRANSIT: [{ status: 'ARRIVED' }],
    ARRIVED: [{ status: 'SIGNED' }],
  };
  const receiptName = sales ? '交付凭证' : '收货凭证';
  return <div className="space-y-6"><div className="flex items-center justify-between"><div className="flex items-center gap-3"><Button variant="ghost" size="icon" onClick={() => router.push('/dashboard/dispatch')}><ArrowLeft className="h-4 w-4" /></Button><div><div className="flex items-center gap-2"><h1 className="text-2xl font-bold">{waybill.waybillNo}</h1><BusinessDirectionBadge type={waybill.dispatchNotice.type} suffix="运输" /><StatusText status={waybill.status}>{waybillStatusLabel(waybill.status, waybill.dispatchNotice.type, Boolean(waybill.plateNo && waybill.driverName))}</StatusText></div><p className="mt-1 text-sm text-muted-foreground">物流运单 · {waybill.freightMode === 'SELF' ? '自有运力' : '第三方承运'}</p></div></div><div className="flex gap-2">{(actions[waybill.status] || []).map(action => { const label = waybillTransitionLabel(action.status, waybill.dispatchNotice.type); const needsReceipt = action.status === 'SIGNED' && !waybill.attachments.some(item => isReceiptAttachment(item.category)); return <Button key={action.status} variant={action.variant || 'default'} disabled={needsReceipt} title={needsReceipt ? `请先上传至少一份${receiptName}` : undefined} onClick={() => void transition(action.status, label)}>{needsReceipt ? `上传${receiptName}后${sales ? '确认客户签收' : '确认收货'}` : label}</Button>; })}</div></div>
    <TransportProgress waybill={waybill} />
    <div className="grid gap-5 lg:grid-cols-3"><Card className="space-y-4 p-5 lg:col-span-2"><h2 className="font-semibold">运输信息</h2><div className="grid grid-cols-2 gap-4 md:grid-cols-3"><Field label="运输数量" value={`${Number(waybill.totalQuantity).toLocaleString()} 吨`} /><Field label="计划发运时间" value={formatDate(waybill.plannedDepartureAt)} /><Field label={sales ? '预计送达时间' : '预计到达时间'} value={formatDate(waybill.plannedArrivalAt)} /><Field label="起运地点" value={waybill.originLocation || '-'} /><Field label="目的地点" value={waybill.destinationLocation || '-'} /><Field label="实际发运" value={formatDate(waybill.departedAt)} /><Field label={sales ? '实际送达' : '实际到达'} value={formatDate(waybill.arrivedAt)} /><Field label={sales ? '客户签收时间' : '收货时间'} value={formatDate(waybill.signedAt)} /><Field label="备注" value={waybill.remarks || '-'} /></div></Card><Card className="p-5"><div className="mb-4 flex items-center gap-2"><FileText className="h-4 w-4 text-primary" /><h2 className="font-semibold">上游执行通知</h2></div><button className="text-left" onClick={() => router.push(`/dashboard/dispatch-notices/${waybill.dispatchNotice.id}`)}><div className="font-medium">{waybill.dispatchNotice.order.name}</div><div className="mt-1 font-mono text-xs text-primary">{waybill.dispatchNotice.noticeNo} · {waybill.dispatchNotice.order.orderNo}</div><div className="mt-1 text-xs text-muted-foreground">{waybill.dispatchNotice.order.contract.contractNo} · {waybill.dispatchNotice.order.contract.title}</div></button></Card></div>
    {waybill.status === 'PENDING' && <Card className="space-y-4 p-5"><div className="flex items-center justify-between"><div><h2 className="font-semibold">车辆与司机调度</h2><p className="text-xs text-muted-foreground">可从车辆和司机档案引用，也可手工填写临时调度信息</p></div><div className="flex gap-2"><Link href="/dashboard/master-data/service-organizations?type=LOGISTICS_CARRIER"><Button variant="outline">维护司机</Button></Link><Link href="/dashboard/master-data/vehicles/new"><Button variant="outline">维护车辆</Button></Link><Button onClick={() => void saveAssignment()}>保存调度</Button></div></div><div className="grid gap-4 md:grid-cols-4"><div><label className="mb-1 block text-sm">运输方式</label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={freightMode} onChange={e => changeFreightMode(e.target.value)}><option value="SELF">自有运力</option><option value="THIRD_PARTY">第三方承运</option></select></div><div><label className="mb-1 block text-sm">车辆</label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={vehicleId} onChange={e => selectVehicle(e.target.value)}><option value="">手工填写</option>{selectableVehicles.map(item => <option key={item.id} value={item.id}>{item.plateNo} · {item.brand || vehicleTypeLabel(item.vehicleType)} · {Number(item.loadCapacity)} 吨{item.owner?.name ? ` · ${item.owner.name}` : ''}</option>)}</select></div><div><label className="mb-1 block text-sm">计划发运时间</label><Input type="datetime-local" value={plannedDepartureAt} onChange={e => setPlannedDepartureAt(e.target.value)} /></div><div><label className="mb-1 block text-sm">预计到达时间</label><Input type="datetime-local" min={plannedDepartureAt || undefined} value={plannedArrivalAt} onChange={e => setPlannedArrivalAt(e.target.value)} /></div>{freightMode === 'THIRD_PARTY' && <div><label className="mb-1 block text-sm">物流承运商 *</label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={carrierPartnerId} onChange={e => { setCarrierPartnerId(e.target.value); setVehicleId(''); setPlateNo(''); setDriverId(''); setDriverSearch(''); setDriverName(''); setDriverPhone(''); }}><option value="">请选择</option>{carriers.map(item => <option key={item.id} value={item.partnerId}>{item.partner.code} · {item.partner.name}</option>)}</select>{canViewLogisticsContract && carrierPartnerId && <Link href={`/dashboard/logistics-contracts?carrierPartnerId=${encodeURIComponent(carrierPartnerId)}&carrierName=${encodeURIComponent(carriers.find(item => item.partnerId === carrierPartnerId)?.partner.name || '')}`} className="mt-1 inline-block text-xs text-primary hover:underline">查看该承运方物流合同</Link>}</div>}<div><label className="mb-1 block text-sm">车牌号 *</label><Input value={plateNo} onChange={e => setPlateNo(e.target.value)} /></div><div><label className="mb-1 block text-sm">司机模糊搜索</label><Input value={driverSearch} onChange={e => setDriverSearch(e.target.value)} placeholder="姓名、手机号、服务商" /></div><div><label className="mb-1 block text-sm">选择司机</label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={driverId} onChange={e => selectDriver(e.target.value)}><option value="">手工填写</option>{selectableDrivers.map(item => <option key={item.id} value={item.id}>{item.name} · {item.phone} · {item.serviceOrganization.partner.name}</option>)}</select></div><div><label className="mb-1 block text-sm">司机姓名 *</label><Input value={driverName} onChange={e => { setDriverId(''); setDriverName(e.target.value); }} /></div><div><label className="mb-1 block text-sm">司机电话</label><Input value={driverPhone} onChange={e => { setDriverId(''); setDriverPhone(e.target.value); }} /></div></div></Card>}
    {waybill.status !== 'PENDING' && <Card className="grid gap-4 p-5 md:grid-cols-4"><Field label="车牌号" value={waybill.plateNo || '-'} /><Field label="司机" value={waybill.driverName || '-'} /><Field label="司机电话" value={waybill.driverPhone || '-'} /><div><Field label="承运单位" value={waybill.carrierName || (waybill.freightMode === 'SELF' ? '自有运力' : '-')} />{canViewLogisticsContract && waybill.carrierPartnerId && <Link href={`/dashboard/logistics-contracts?carrierPartnerId=${encodeURIComponent(waybill.carrierPartnerId)}&carrierName=${encodeURIComponent(waybill.carrierName || '')}`} className="mt-2 inline-block text-xs text-primary hover:underline">查看该承运方物流合同</Link>}</div></Card>}
    {['ARRIVED', 'SIGNED'].includes(waybill.status) && <section className="space-y-3"><Card className="p-5"><h2 className="font-semibold">{sales ? '物流交付凭证' : '物流收货凭证'}</h2><p className="mt-1 text-sm text-muted-foreground">{sales ? '确认客户签收' : '确认收货'}前，以下三类附件合计至少上传一份；无需每类都上传。</p></Card><AttachmentPanel title={sales ? '客户签收单' : '收货签收单'} description={sales ? '上传客户签字或盖章的签收单，支持图片或 PDF' : '上传我方签字或盖章的收货单，支持图片或 PDF'} attachments={waybill.attachments.filter(item => item.category === 'RECEIPT_DOCUMENT')} uploadPath={`/waybills/${waybill.id}/attachments`} uploadCategory="RECEIPT_DOCUMENT" uploadLabel={sales ? '上传客户签收单' : '上传收货签收单'} attachmentPath="/waybills/attachments" canUpload canDelete={waybill.status === 'ARRIVED'} onChanged={load} /><AttachmentPanel title={sales ? '送达现场照片' : '收货现场照片'} description={sales ? '上传客户现场、卸货、货物或车辆照片' : '上传卸货、货物、车辆或现场交接照片'} attachments={waybill.attachments.filter(item => item.category === 'RECEIPT_PHOTO')} uploadPath={`/waybills/${waybill.id}/attachments`} uploadCategory="RECEIPT_PHOTO" uploadLabel="上传现场照片" attachmentPath="/waybills/attachments" canUpload canDelete={waybill.status === 'ARRIVED'} onChanged={load} /><AttachmentPanel title={sales ? '其他交付凭证' : '其他收货凭证'} description={sales ? '上传交接记录、客户回单等补充凭证' : '上传交接记录等补充收货凭证；历史附件也归入此处'} attachments={waybill.attachments.filter(item => ['RECEIPT_OTHER', 'RECEIPT'].includes(item.category))} uploadPath={`/waybills/${waybill.id}/attachments`} uploadCategory="RECEIPT_OTHER" uploadLabel="上传其他凭证" attachmentPath="/waybills/attachments" canUpload canDelete={waybill.status === 'ARRIVED'} onChanged={load} /></section>}
    {waybill.status !== 'CANCELLED' && <Card className="flex flex-wrap items-center justify-between gap-4 p-5"><div><h2 className="font-semibold">物流称重与{sales ? '结算出库磅单' : '结算入库磅单'}</h2><p className="mt-1 text-xs text-muted-foreground">该运单已归集 {waybill.weighTickets.filter(item => item.status !== 'VOIDED').length} 张发货/收货称重磅单；从中统一选择一张作为库存执行和结算依据。</p><div className="mt-3"><WeightSelectionSummary waybill={waybill} /></div></div><div className="flex gap-2"><Button variant="outline" onClick={() => router.push(`/dashboard/weighbridge/create?waybillId=${waybill.id}`)}>新增称重磅单</Button><Button onClick={() => router.push(`/dashboard/weighbridge/management/${waybill.id}`)}>查看磅单信息</Button></div></Card>}
    {['ARRIVED', 'SIGNED'].includes(waybill.status) && (waybill.qualityTask || !sales) && <Card className="flex flex-wrap items-center justify-between gap-4 p-5"><div><h2 className="font-semibold">{sales ? '交付质检任务' : '到货质检任务'}</h2>{waybill.qualityTask ? <p className="mt-1 text-sm"><span className="font-mono text-primary">{waybill.qualityTask.taskNo}</span> · 已归集 {waybill.qualityTask._count.reports} 份检测报告 · {waybill.qualityTask.status === 'COMPLETED' ? '已完成最终判定' : '待质检管理人员处理'}</p> : <p className="mt-1 text-sm text-amber-700">任务正在自动生成，请刷新后查看。</p>}</div>{waybill.qualityTask && <Button onClick={() => router.push(`/dashboard/quality/${waybill.qualityTask!.id}`)}>查看质检任务</Button>}</Card>}
...
function WeightSelectionSummary({ waybill }: { waybill: Waybill }) {
  ...
  return <SelectionField label={`${waybill.dispatchNotice.type === 'SALES' ? '结算出库磅单' : '结算入库磅单'}（库存与结算统一口径）`} selection={selection} ticket={ticket} />;
}
...
function TransportProgress({ waybill }: { waybill: Waybill }) {
  ...
  const steps = [
    ...
    { status: 'ARRIVED', label: sales ? '已送达' : '已到达', icon: MapPin, time: waybill.arrivedAt },
    { status: 'SIGNED', label: sales ? '客户已签收' : '已收货', icon: CheckCircle2, time: waybill.signedAt },
  ];
