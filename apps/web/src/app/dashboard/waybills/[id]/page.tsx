...
export default function WaybillDetailPage() {
  ...
  return <div className="space-y-6"><div className="flex items-center justify-between"><div className="flex items-center gap-3"><Button variant="ghost" size="icon" onClick={() => router.push('/dashboard/dispatch')}><ArrowLeft className="h-4 w-4" /></Button><div><div className="flex items-center gap-2"><h1 className="text-2xl font-bold">{waybill.waybillNo}</h1><Badge>{STATUS[waybill.status]}</Badge></div><p className="mt-1 text-sm text-muted-foreground">物流运单 · {waybill.freightMode === 'SELF' ? '自有运力' : '第三方承运'}</p></div></div><div className="flex gap-2">{(ACTIONS[waybill.status] || []).map(action => { const needsReceipt = action.status === 'SIGNED' && !waybill.attachments.some(item => isReceiptAttachment(item.category)); return <Button key={action.status} variant={action.variant || 'default'} disabled={needsReceipt} title={needsReceipt ? '请先上传至少一份收货附件' : undefined} onClick={() => void transition(action.status, action.label)}>{needsReceipt ? '上传收货附件后签收' : action.label}</Button>; })}</div></div>
    ...
    {['ARRIVED', 'SIGNED'].includes(waybill.status) && <section className="space-y-3"><Card className="p-5"><h2 className="font-semibold">物流收货附件</h2><p className="mt-1 text-sm text-muted-foreground">确认签收前，以下三类附件合计至少上传一份；无需每类都上传。</p></Card><AttachmentPanel title="签收单" description="上传签字或盖章的签收单，支持图片或 PDF" attachments={waybill.attachments.filter(item => item.category === 'RECEIPT_DOCUMENT')} uploadPath={`/waybills/${waybill.id}/attachments`} uploadCategory="RECEIPT_DOCUMENT" uploadLabel="上传签收单" attachmentPath="/waybills/attachments" canUpload canDelete={waybill.status === 'ARRIVED'} onChanged={load} /><AttachmentPanel title="收货现场照片" description="上传卸货、货物、车辆或现场交接照片" attachments={waybill.attachments.filter(item => item.category === 'RECEIPT_PHOTO')} uploadPath={`/waybills/${waybill.id}/attachments`} uploadCategory="RECEIPT_PHOTO" uploadLabel="上传现场照片" attachmentPath="/waybills/attachments" canUpload canDelete={waybill.status === 'ARRIVED'} onChanged={load} /><AttachmentPanel title="其他交接凭证" description="上传交接记录、客户回单等补充凭证；历史收货附件也归入此处" attachments={waybill.attachments.filter(item => ['RECEIPT_OTHER', 'RECEIPT'].includes(item.category))} uploadPath={`/waybills/${waybill.id}/attachments`} uploadCategory="RECEIPT_OTHER" uploadLabel="上传其他凭证" attachmentPath="/waybills/attachments" canUpload canDelete={waybill.status === 'ARRIVED'} onChanged={load} /></section>}
