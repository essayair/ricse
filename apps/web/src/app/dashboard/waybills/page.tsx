...
import { BusinessDirectionBadge, businessDirectionStyle, waybillStatusLabel } from '@/components/business-direction';
...
const STATUS: Record<string, string> = { PENDING: '待调度/待发运', IN_TRANSIT: '在途', ARRIVED: '已到目的地待交接', SIGNED: '已完成交接', CANCELLED: '已取消' };
...
export default function WaybillsPage() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold">物流运单管理</h1><p className="mt-1 text-sm text-muted-foreground">按执行通知拆分车次，完成车辆调度、运输跟踪和签收</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => router.push('/dashboard/waybills/batch-create')}><ListPlus className="mr-1 h-4 w-4" />批量派车</Button><Button onClick={() => router.push('/dashboard/waybills/create')}><Plus className="mr-1 h-4 w-4" />单车建单</Button></div></div>
    ...
    <Card className="overflow-hidden">{!items.length ? <div className="p-12 text-center text-muted-foreground">暂无物流运单</div> : <div className="overflow-x-auto"><table className="min-w-[1380px] w-full text-sm"><thead className="border-b bg-muted/50 text-left text-muted-foreground"><tr><th className="px-4 py-3">物流运单 / 计划</th><th className="px-4 py-3">通知 / 批次 / 合同</th><th className="px-4 py-3">运输方式 / 承运方</th><th className="px-4 py-3">车辆 / 司机</th><th className="px-4 py-3">运输路线</th><th className="px-4 py-3 text-right">标的 / 数量</th><th className="px-4 py-3">运输时间</th><th className="px-4 py-3">状态 / 创建人</th></tr></thead><tbody>{items.map(item => { const sales = item.dispatchNotice.type === 'SALES'; return <tr key={item.id} className={`cursor-pointer border-b transition-colors ${businessDirectionStyle(item.dispatchNotice.type).hover}`} onClick={() => router.push(`/dashboard/waybills/${item.id}`)}><td className="px-4 py-3"><div className="mb-1"><BusinessDirectionBadge type={item.dispatchNotice.type} suffix="运输" /></div><div className="font-mono text-xs font-medium">{item.waybillNo}</div><div className="mt-1 text-xs text-muted-foreground">发运 {formatDateTime(item.plannedDepartureAt)}</div><div className="mt-1 text-xs text-muted-foreground">{sales ? '送达' : '到达'} {formatDateTime(item.plannedArrivalAt)}</div></td><td className="max-w-64 px-4 py-3"><div className="truncate font-medium">{item.dispatchNotice.order.name}</div><div className="mt-1 font-mono text-xs text-muted-foreground">{item.dispatchNotice.noticeNo} · {item.dispatchNotice.order.orderNo}</div><div className="mt-1 truncate text-xs text-muted-foreground">{item.dispatchNotice.order.contract.contractNo} · {item.dispatchNotice.order.contract.title}</div></td><td className="px-4 py-3"><div>{item.freightMode === 'SELF' ? '自营运输' : '委外运输'}</div><div className="mt-1 text-xs text-muted-foreground">{item.carrierName || '未指定承运方'}</div></td><td className="px-4 py-3"><div>{item.plateNo || '待调度'}</div><div className="mt-1 text-xs text-muted-foreground">{item.driverName || '-'}{item.driverPhone ? ` · ${item.driverPhone}` : ''}</div></td><td className="max-w-64 px-4 py-3 text-xs"><div className="truncate">{item.originLocation || '-'}</div><div className="mt-1 truncate text-muted-foreground">→ {item.destinationLocation || '-'}</div></td><td className="px-4 py-3 text-right"><div className="max-w-40 truncate">{item.lineItems?.[0]?.materialName || '-'}</div><div className="mt-1 text-xs text-muted-foreground">{waybillQuantity(item)}</div></td><td className="px-4 py-3 text-xs"><div>发运：{formatDateTime(item.departedAt)}</div><div className="mt-1 text-muted-foreground">{sales ? '送达' : '到达'}：{formatDateTime(item.arrivedAt)}</div><div className="mt-1 text-muted-foreground">{sales ? '客户签收' : '收货'}：{formatDateTime(item.signedAt)}</div></td><td className="px-4 py-3"><StatusText status={item.status}>{waybillStatusLabel(item.status, item.dispatchNotice.type, Boolean(item.plateNo && item.driverName))}</StatusText><div className="mt-1 text-xs text-muted-foreground">{item.creator?.name || '-'} · {formatDateTime(item.createdAt, false)}</div></td></tr>; })}</tbody></table></div>}</Card>
  </div>;
}
