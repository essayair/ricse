...
export default function CarrierDriversPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="overflow-hidden">{!drivers.length ? <div className="p-12 text-center text-muted-foreground">暂无司机档案</div> : <div className="overflow-x-auto"><table className="min-w-[1000px] w-full text-sm"><thead className="border-b bg-muted/50 text-left text-muted-foreground"><tr><th className="p-3">司机 / 手机</th><th className="p-3">身份证号</th><th className="p-3">驾驶证 / 准驾车型</th><th className="p-3">有效期</th><th className="p-3">引用运单</th><th className="p-3">状态</th><th className="p-3">操作</th></tr></thead><tbody>{drivers.map(driver => <tr key={driver.id} className="border-b"><td className="p-3"><div className="font-medium">{driver.name}</div><div className="mt-1 text-xs text-muted-foreground">{driver.phone}</div></td><td className="p-3">{driver.idCardNo || '—'}</td><td className="p-3"><div>{driver.licenseNo || '—'}</div><div className="mt-1 text-xs text-muted-foreground">{driver.licenseClass || '未填写'}</div></td><td className="p-3">{driver.licenseExpiry?.slice(0, 10) || '—'}</td><td className="p-3">{driver._count.waybills} 单</td><td className="p-3"><StatusText status={driver.status}>{driver.status === 'ACTIVE' ? '启用' : '停用'}</StatusText></td><td className="p-3"><div className="flex gap-1"><Button size="sm" variant="ghost" onClick={() => edit(driver)}>编辑</Button><Button size="sm" variant="ghost" onClick={() => void toggle(driver)}>{driver.status === 'ACTIVE' ? '停用' : '启用'}</Button></div></td></tr>)}</tbody></table></div>}</Card>
  </div>;
}
