  ...
  return <div className="space-y-6">
    ...
    <Card className="overflow-hidden">
      ...
      <div className="overflow-x-auto"><table className="min-w-[1100px] w-full text-sm"><thead className="border-b bg-muted/50 text-left text-muted-foreground"><tr><th className="p-3">档案编码</th><th className="p-3">合作伙伴</th><th className="p-3">资质信息</th><th className="p-3">服务范围 / 区域</th><th className="p-3">联系人</th><th className="p-3">状态</th><th className="p-3">操作</th></tr></thead><tbody>
        {items.map(item => <tr key={item.id} className="border-b"><td className="p-3 font-mono">{item.code}</td><td className="p-3"><div className="font-medium">{item.partner.name}</div><div className="mt-1 text-xs text-muted-foreground">{item.partner.code} · 供应商</div></td><td className="p-3"><div>{item.licenseNo || item.qualificationNo || item.cmaNo || '—'}</div><div className="mt-1 text-xs text-muted-foreground">{item.licenseExpiry ? `有效期至 ${item.licenseExpiry.slice(0, 10)}` : item.cnasNo ? `CNAS ${item.cnasNo}` : '未设置有效期'}</div></td><td className="max-w-80 p-3"><div className="truncate">{item.serviceScope || operationLabel(item.operationType) || '—'}</div><div className="mt-1 truncate text-xs text-muted-foreground">{item.serviceRegions || '未设置服务区域'}</div></td><td className="p-3"><div>{item.contactPerson || '—'}</div><div className="mt-1 text-xs text-muted-foreground">{item.contactPhone || '—'}</div></td><td className="p-3"><Badge variant={item.status === 'ACTIVE' ? 'default' : 'secondary'}>{statusLabel(item.status)}</Badge></td><td className="p-3"><div className="flex gap-1">{item.organizationType === 'LOGISTICS_CARRIER' && <Link href={`/dashboard/master-data/service-organizations/${item.id}/drivers`}><Button size="sm" variant="outline">司机</Button></Link>}<Link href={`/dashboard/master-data/service-organizations/${item.id}/edit`}><Button size="sm" variant="ghost">编辑</Button></Link><Button size="sm" variant="ghost" onClick={() => void updateStatus(item)}>{item.status === 'ACTIVE' ? '停用' : '启用'}</Button></div></td></tr>)}
      </tbody></table></div>}
    </Card>
  </div>;
}
