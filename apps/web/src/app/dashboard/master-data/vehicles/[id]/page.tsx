...
const WEIGH_TICKET_STATUS: Record<string, string> = { DRAFT: '草稿', COMPLETED: '已完成', REVIEWED: '已复核', VOIDED: '已作废' };
...
export default function VehicleDetailPage() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><Button variant="ghost" size="icon" onClick={() => router.push('/dashboard/master-data?tab=vehicles')}><ArrowLeft className="h-4 w-4" /></Button><div><div className="flex items-center gap-2"><h1 className="text-2xl font-bold">{vehicle.plateNo}</h1><StatusText status={vehicle.status}>{STATUS[vehicle.status] || vehicle.status}</StatusText></div><p className="mt-1 text-sm text-muted-foreground">{VEHICLE_TYPES[vehicle.vehicleType] || vehicle.vehicleType} · {vehicle.owner?.name || (vehicle.ownerType === 'SELF' ? '自有车辆' : '未设置承运商')}</p></div></div><div className="flex gap-2"><Button variant="outline" onClick={() => router.push(`/dashboard/master-data/vehicles/${id}/edit`)}><Pencil className="mr-1 h-4 w-4" />编辑</Button>{vehicle.status !== 'RETIRED' && <Button variant="outline" onClick={() => void toggleStatus()}>{vehicle.status === 'ACTIVE' ? '转维修停用' : '恢复可用'}</Button>}</div></div>
