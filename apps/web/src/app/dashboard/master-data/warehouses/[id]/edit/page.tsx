...
export default function WarehouseEditPage() {
  ...
  useEffect(() => {
    api.get<{ items: Array<{ id: string; partnerId: string; partner: { code: string; name: string } }> }>('/service-organizations?type=WAREHOUSE_PORT&status=ACTIVE&pageSize=200')
      ...
      .catch(() => {});
    api.get<any>(`/master-data/warehouses/${id}`).then((w) => {
      setCode(w.code || '');
      setName(w.name || '');
      setType(w.type || 'SELF');
      setPartnerId(w.partnerId || '');
      setAddress(w.address || '');
      setManager(w.manager || '');
      setManagerPhone(w.managerPhone || '');
      setStatus(w.status || 'ACTIVE');
      setRemark(w.remark || '');
    }).catch(() => setError('加载仓库信息失败'));
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      await api.patch(`/master-data/warehouses/${id}`, {
        name,
        type,
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">编辑仓库</h1>
            <p className="text-sm text-muted-foreground mt-0.5">修改仓库信息 · 编码：{code}</p>
          </div>
        </div>
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-4">
            <FormField label="仓库编码" required>
              <Input value={code} disabled className="font-mono bg-muted/50" />
            </FormField>
