...
import { ArrowLeft, FileText, ListPlus, Truck, Pencil } from 'lucide-react';
import { api } from '@/lib/api';
import { Button } from '@/components/ui/button';
import { Card } from '@/components/ui/card';
import { unitLabel } from '@/lib/unit';
import { BusinessOperationHistory } from '@/components/business-operation-history';
import { BusinessDirectionBadge, waybillStatusLabel } from '@/components/business-direction';
import { StatusText } from '@/components/status-text';

interface Notice {
  id: string; noticeNo: string; type: string; mode: string; status: string; qualityRequired: boolean; totalQuantity: string;
  plannedDate: string | null; originLocation: string | null; destinationLocation: string | null;
  originContactPerson: string | null; originContactPhone: string | null; destinationContactPerson: string | null; destinationContactPhone: string | null;
  ...
  waybills: Array<{ id: string; waybillNo: string; status: string; totalQuantity: string; plateNo: string | null; driverName: string | null }>;
}
...
export default function DispatchNoticeDetailPage() {
  ...
  return <div className="space-y-6">
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-3"><Button variant="ghost" size="icon" onClick={() => router.push('/dashboard/dispatch-notices')}><ArrowLeft className="h-4 w-4" /></Button><div><div className="flex items-center gap-2"><h1 className="text-2xl font-bold">{notice.noticeNo}</h1><BusinessDirectionBadge type={notice.type} suffix="通知" /><StatusText status={notice.status}>{STATUS[notice.status]}</StatusText></div><p className="mt-1 text-sm text-muted-foreground">{notice.type === 'PURCHASE' ? '供应商发货指令' : '销售发货通知单'} · {notice.mode === 'DIRECT' ? '直拨' : '常规'}</p></div></div>
      <div className="flex gap-2">
        {['DRAFT', 'ISSUED'].includes(notice.status) && notice.waybills.length === 0 && <Button variant="outline" onClick={() => router.push(`/dashboard/dispatch-notices/${notice.id}/edit`)}><Pencil className="mr-1 h-4 w-4" />修改地址</Button>}
    ...
    <div className="grid gap-5 lg:grid-cols-3">
      <Card className="space-y-4 p-5 lg:col-span-2"><h2 className="font-semibold">通知信息</h2><div className="grid grid-cols-2 gap-4 text-sm">
        ...
        <Field label="起运联系人" value={[notice.originContactPerson, notice.originContactPhone].filter(Boolean).join(' · ') || '-'} /><Field label="目的地联系人" value={[notice.destinationContactPerson, notice.destinationContactPhone].filter(Boolean).join(' · ') || '-'} />
        <Field label="发货仓库" value={notice.warehouse ? `${notice.warehouse.code} · ${notice.warehouse.name}` : '-'} /><Field label={notice.type === 'SALES' ? '交付质检' : '到货质检'} value={notice.qualityRequired ? '需要，到目的地后自动生成任务' : '不需要'} /><Field label="创建人" value={notice.creator.name} />
